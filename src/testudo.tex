...
\item ``\texttt{uc}'' (uppercase), where all macro names are in uppercase, and
  ...
  \input{examples/styles-uc-op.tex}
\end{itemize}
...
\tref{tab:make-commands} shows the available ``\texttt{make}'' commands.  In
...
\item the argument to the ``true'' and ``false'' syntaxes
  (\Sref{sec:check-expression-true}, \Sref{sec:check-expression-false});
...
\item the argument to the ``verify'' and ``not-verify'' syntaxes
  (\Sref{sec:predicate-checks});
\item the argument to the ``true-for'' and ``false-for'' syntaxes
  (\Sref{sec:check-expression-true-for},
  \Sref{sec:check-expression-false-for}).
\end{itemize}
