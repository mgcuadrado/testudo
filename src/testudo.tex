...
boolean.  A predicate can be constructed using in one of three ways:
\begin{itemize}
...
\item from a boolean expression:
...
\item from a boolean expression with capture:
...
thrown, the test step is failed, and an unexpected exception error is reported (see~\Sref{sec:unexpected-exceptions}).
...
of things you can do to investigate what is going in.  First, you can run the
...
(use one of them as a template, and adapt the second half of each line).
