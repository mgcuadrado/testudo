...
//     string const color_failure="\033[1;31m";
  ...
  private:
    ...
    void format_fill(ostream &stream, char fill,
      ...
      stream << format_text_prefix
             << pre_color
             << string(separator_width-real_length(format_text_prefix), fill)
             << post_color << endl;
    }
  ...
  public:
    ...
    void output_title(string name, string title) override {
      ...
      ostringstream oss;
      oss << name_open << brief_name << name_close
    ...
    void output_location() {
      ...
      // location info
      if (show_location)
        if (string brief_location=get_brief_location();
            not brief_location.empty())
          set_format_text_prefix(loc_begin+brief_location+loc_end+" ");
      set_location({});
    }
    ...
    void output_separator() override {
      output_location();
      format_fill(ts, '-', color_lines, color_normal);
    }
    ...
    void output_begin_with(string var_name,
      ...
      output_location();
      ostringstream oss;
      oss << check_verify << " " << var_name << " in " << container_first;
      format_text(ts, oss.str());
    ...
    void output_check_true_for(string expr_str,
      ...
      output_check_general(
        ...
        {expr_str},
        success,
        {"false", show_ident, exprv_str, show_sep, valv_str});
    }
